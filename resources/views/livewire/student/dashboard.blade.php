...
<div class="space-y-6">
    ...
    @else
        <!-- My Courses -->
        <h2 class="text-xl font-bold text-zinc-900 mt-8 mb-4">My Courses</h2>
            ...
            @foreach($enrollments as $enrollment)
                @if($enrollment->course)
                    <div class="group relative bg-white border border-zinc-200 rounded-xl p-5 hover:border-primary-500 hover:shadow-md transition-all">
                        ...
                        <h5 class="text-lg font-bold text-zinc-900 mb-1">{{ $enrollment->course->name }}</h5>
                        <p class="text-sm text-zinc-500 line-clamp-2 mb-2">{{ $enrollment->course->description }}</p>
                        <p class="text-xs text-zinc-400 mb-4">Term: {{ $enrollment->term->name ?? 'N/A' }}</p>
                        ...
                        <div class="mt-4 pt-4 border-t border-zinc-100 flex items-center justify-between">
                            ...
                            <a href="{{ route('student.course.viewer', $enrollment->course->id) }}" class="text-sm font-semibold text-primary-600 hover:text-primary-700">Enter Class &rarr;</a>
                        </div>
                    </div>
                @endif
            @endforeach
