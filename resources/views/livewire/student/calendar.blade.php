            ...
            if (!empty($courseIds)) {
                ...
                $this->upcomingSessions = $this->sessions
                    ->filter(function ($session) {
                        return Carbon::parse($session->session_date->format('Y-m-d') . ' ' . $session->session_start_time)->isFuture();
                    })
                    ->take(5);
            }
        }
...
<div class="space-y-6">
    <!-- Header -->
    <div
        class="bg-white rounded-2xl shadow-sm border border-zinc-100 p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between">
        <div>
        ...
        <div
            class="mt-4 sm:mt-0 px-4 py-2 bg-primary-50 rounded-lg text-primary-700 font-medium text-sm flex items-center">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            @else
                <div class="space-y-4">
                    ...
                    @foreach ($sessions as $session)
                        ...
                        @if ($sessionDate !== $currentDate)
                            <h3 class="font-semibold text-zinc-900 mt-6 mb-2 flex items-center">
                                ...
                                @if (\Carbon\Carbon::parse($session->session_date)->isToday())
                                    <span
                                        class="ml-3 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-primary-100 text-primary-800">Today</span>
                                @endif
                            </h3>
                            @php
                        ...
                        <div
                            class="bg-white border border-zinc-200 rounded-xl p-5 hover:border-primary-300 hover:shadow-md transition-all flex flex-col sm:flex-row sm:items-center gap-4">
                            <!-- Time Column -->
                            <div class="flex-shrink-0 w-32 border-l-4 border-primary-500 pl-4 py-1">
                                <div class="text-sm font-bold text-zinc-900">
                                    {{ $this->formatTime($session->session_start_time) }}</div>
                                <div class="text-xs text-zinc-500 mt-1">
                                    {{ $this->formatTime($session->session_end_time) }}</div>
                                <div class="text-xs font-medium text-primary-600 mt-2">{{ $session->timezone ?? 'EST' }}
                                </div>
                            </div>

                            <!-- Detail Column -->
                            <div class="flex-1 min-w-0">
                                <h4 class="text-lg font-bold text-zinc-900 truncate">
                                    {{ $session->course->name ?? 'Course Name' }}</h4>
                                ...
                                <div class="mt-1 flex items-center text-xs text-zinc-500">
                                    @if ($session->platform === 'zoom')
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800 mr-2">Zoom
                                            Session</span>
                                    @elseif($session->platform === 'start_meeting')
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800 mr-2">StartMeeting
                                            Session</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="flex-shrink-0 self-start sm:self-center">
                                @if ($session->meeting_url)
                                    <a href="{{ $session->meeting_url }}" target="_blank"
                                        class="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-primary-600 hover:bg-primary-700 shadow-sm transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                                        Join Class
                                        <svg class="ml-2 -mr-1 h-4 w-4" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                @else
                                    <button disabled
                                        class="inline-flex items-center justify-center px-4 py-2 border border-zinc-200 text-sm font-medium rounded-lg text-zinc-400 bg-zinc-50 cursor-not-allowed">
                                        Link Pending
                                    </button>
                                @endif
                            </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-zinc-100 overflow-hidden">
                <div class="p-6 border-b border-zinc-100 bg-zinc-50/50">
                    <h3 class="text-lg font-bold text-zinc-900 flex items-center">
                        <svg class="mr-2 h-5 w-5 text-primary-500" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <div class="flex justify-between items-start">
                                        <div class="min-w-0">
                                            ...
                                            <p class="text-xs text-zinc-500 mt-1 flex items-center">
                                                ...
                                                {{ \Carbon\Carbon::parse($upcoming->session_date)->format('M j') }} at
                                                {{ $this->formatTime($upcoming->session_start_time) }}
                                            </p>
                                        </div>
                                    </div>
                                </li>
            ...
            <div class="bg-primary-50 rounded-2xl border border-primary-100 p-6">
                ...
                <p class="text-sm text-primary-700 mb-4">If you're having trouble accessing a live session, make sure
                    you're logged into the correct Zoom or StartMeeting account.</p>
